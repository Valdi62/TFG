...
class CustomImageDataset(Dataset):
    ...
    tendrá etiquetas para cada imagen con las coordenadas de las esquinas en dichas imágenes en caso de querer usarse para el entrenamiento del modelo.
    ...
    def __init__(self,images_dir,df,train=False):
        ...
        self.transform = transforms.Compose([
                         ...
                         transforms.Normalize([0.485, 0.456, 0.406],[0.229, 0.224, 0.225]) # Realiza la misma normalización que se aplica a las imagenes de imagenet
                         ])
        # Obtenemos las etiquetas solo si queremos usar el dataset para el proceso de entrenamiento del modelo
        self.train = train
    ...
    def __getitem__(self,idx):
        ...
        image = read_image(img_path) # De esta forma las imágenes se pasan directamente a tensor
        ...
        # Se comprueba si se tienen las etiquetas para devolver el conjunto completo para entreamiento o solo las imágenes
        if self.train:
            has_corners = self.labels[idx][0]
            coords = self.labels[idx][1:]
            has_corners = torch.tensor(has_corners)

            # Normalizamos las coordenadas en función de dimensiones fijas porque son las que se usaro durante el etiquetado,
            # es decir todas las fotos se rotaron y ajustaron en tamaño antes de generar sus etiquetas
            ...
            # En caso de no tener etiquetas se devuelve la imágen junto con su nombre
...
def corners_dataset(df,target_col,out_dir,train_size=0.7):
    ...
    Se almacenarán en formato csv con esos nombres junto con '_crop' añadido al final, en un directorio de salida elegido
    ...
    """
    # Fijamos una semilla para replicabilidad
...
def main():
    """
    Desde el main de este script se crea un conjunto de entrenamiento, validación y test utilizando
